	...
	if (updateCodeMenu) {
		...
		if (canMove) {
			...
			if (FlxG.mouse.overlaps(codeListOpenHitbox)) {
				...
				cursor = "ibeam";
				if (FlxG.mouse.justPressed) FlxG.stage.window.textInputEnabled = true;
				if (FlxG.mouse.justReleased) 
				{
...
var CodesFunctions:{} = {
	...
	catbot: function() {
#if mobile
 progInfoText.text = (FlxG.save.data.spanish) ? "Quieres HABILITAR Botplay?" : "Do You Want TO ENABLE Botplay?";

#else
progInfoText.text = (FlxG.save.data.spanish) ? "Te Gustaria Activar Botplay Al Presionar 6 En Una mobile?" : "Enable Activating Botplay When Pressing 6 Mid Song?";
#end
		canMove = true;
		codesFocused = false;
		updateCodeMenu = false;

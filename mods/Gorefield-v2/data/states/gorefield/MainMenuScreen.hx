...
function update(elapsed:Float) {
	...
	if ((FlxG.keys.justPressed.SEVEN || virtualPad.buttonX.justPressed) && FlxG.save.data.dev) {
		persistentUpdate = !(persistentDraw = true);
		openSubState(new EditorPicker());
	}

	if (#if mobile virtualPad.buttonY.justPressed #else controls.SWITCHMOD #end) {
